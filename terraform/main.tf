...
resource "aws_key_pair" "ssh" {
  ...
  provisioner "local-exec" {
    command = "echo '${tls_private_key.ssh.private_key_pem}' > ../ansible/${var.generated_key_name}.pem"
  }

  provisioner "local-exec" {
    command = "chmod 400 ../ansible/${var.generated_key_name}.pem"
  }
}
...
resource "null_resource" "configuration_of_instances" {
  provisioner "local-exec" {
    command = "echo '[mysql]\nserver1 ansible_host=${aws_instance.mysql_instance.private_ip}\n[mysql:vars]\nansible_ssh_user=ubuntu\nansible_ssh_private_key_file=./${var.generated_key_name}.pem' > ../ansible/hosts.txt"
  }
  provisioner "file" {
    source      = "../ansible/"
    ...
    connection {
      ...
      private_key = file("../ansible/${var.generated_key_name}.pem")
    }
 } 
